...
const { deleteTodo, deleteUser } = require('../../src/database/queries/deleteData');
...
describe('Database Delete Data test', () => {
  test('Delete Todo Test', async () => {
    const { rowCount } = await deleteTodo('9c271a2d-beab-41d1-a5ea-b3ea06912742');
    expect(rowCount).toBe(1);
  });

  test('Delete user Test', async () => {
    const { rowCount } = await deleteUser('b3ea641e-1281-435c-8af7-059386395338');
    expect(rowCount).toBe(1);
  });
});
